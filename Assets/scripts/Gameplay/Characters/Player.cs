...
//		GetComponent<BoxCollider>().center = new Vector3(0f,0f,0f);
	...
	private void checkInput()
	{
		...
		if (/*!jumpLocked &&*/ (Input.GetKey(InputMan.Up)  || Input.GetKey(InputMan.Up2) || Input.GetKey(InputMan.UpSpace) || Input.GetKey(InputMan.PadJump))) 
		{
			isJump = true;
		}
		if (!jumpLocked && (Input.GetKeyDown(InputMan.Up)  || Input.GetKeyDown(InputMan.Up2) || Input.GetKey(InputMan.UpSpace) || Input.GetKeyDown(InputMan.PadJump)) /* && grounded*/) 
		{ 
			MasterAudio.PlaySound("player_jump");
			jumpLocked=true;
		}
		if (Input.GetKeyUp(InputMan.Up) || Input.GetKeyUp(InputMan.Up2) || Input.GetKey(InputMan.UpSpace) || Input.GetKeyUp(InputMan.PadJump) ) {jumpLocked = false;chute=true;}
