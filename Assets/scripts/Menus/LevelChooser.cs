...
public class LevelChooser : MonoBehaviour {
	...
	public void lockOccButtons(string numLevel) {
		foreach (MiscButton _mBtn in ArrMiscBtns) {
			if(_mBtn.buttonType == MiscButton.buttonList.PlayOccurence) {
				_mBtn.meLocked = _mBtn.locked = (_mBtn._playerDataLoader.getValueFromXmlDoc("BlobMinute/players/Bastien/level"+numLevel+"/occ"+((_mBtn.numOccurence==6)?"Boss":_mBtn.numOccurence.ToString()) as string,"locked")=="true")?true:false;
